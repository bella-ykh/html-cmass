...
async function loadSportsData() {
    try {
        ...
        console.error('스포츠 데이터 로드 실패:', error);
        return null;
    }
}
...
function generateVideoHTML(video) {
    return `
        <div class="video-item">
            <div class="video-thumbnail" data-video-url="${video.videoUrl}" data-video-title="${video.title}">
...
function generateTechniqueHTML(technique, sportId, techniqueIndex) {
    const videoGridHTML = technique.videos.map(video => generateVideoHTML(video)).join('');
    
    return `
        <div class="sports-video" id="${sportId}-video-${techniqueIndex + 1}">
...
function generateSportHTML(sport, category) {
    const techniqueButtonsHTML = sport.techniques.map((technique, index) => {
        return `<li><button class="detail-btn" data-target="${sport.id}-video-${index + 1}">${technique.name}</button></li>`;
    }).join('');

    const techniquesHTML = sport.techniques.map((technique, index) => 
        generateTechniqueHTML(technique, sport.id, index)
    ).join('');

    // 기술 개수에 따른 클래스 추가
    const techniqueCount = sport.techniques.length;
    const countClass = `count-${techniqueCount}`;
        ...
        <div class="sports-section">
            <div class="sports-menu">
                ...
                <div class="sports-header">
                    <div class="sports-thumbnail">
                        <img src="${sport.thumbnail}" alt="${sport.name}">
                    </div>
                    <div class="sports-title">
                        <h2>${sport.name}</h2>
                        <button class="accordion-toggle" data-target="${sport.id}-videos">
                            <img src="images/ico-open.svg" alt="열기" class="icon-open">
                            <img src="images/ico-close.svg" alt="닫기" class="icon-close">
                        </button>
                    </div>
                </div>
                ...
                <div class="sports-nav">
                    <ul class="sports-list ${countClass}">
                        ${techniqueButtonsHTML}
                    </ul>
                </div>
            </div>
            
            <!-- 아코디언으로 열리는 비디오 영역 -->
            <div class="sports-videos" id="${sport.id}-videos">
                ${techniquesHTML}
            </div>
        </div>
    `;
...
async function renderSports() {
    if (!sportsData) {
        console.error('스포츠 데이터가 로드되지 않았습니다.');
        return;
    }

    // 전략형 스포츠 렌더링
    const strategicContainer = document.getElementById('strategic');
    const strategicHTML = sportsData.strategic.map(sport => generateSportHTML(sport, 'strategic')).join('');
    strategicContainer.innerHTML = strategicHTML;

    // 기술형 스포츠 렌더링
    const technicalContainer = document.getElementById('technical');
    const technicalHTML = sportsData.technical.map(sport => generateSportHTML(sport, 'technical')).join('');
    technicalContainer.innerHTML = technicalHTML;

    console.log('스포츠 섹션 렌더링 완료');
}
